...
export default class MarketOrderApprovalTurnCarmodel extends mixins(ctjtPaginationMixins, ctjttablefieldMixins) {
  ...
  /** 搜索筛选框选择回调 */
  async searchSelectChange(val: ParamsType) {
    const { value, key } = val;
    if (key === 'regionId') {
      this.searchForm.selectList[1].options = [];
  ...
  // 跳转详情
  private jumpDetail(id?: string, orderId?: string, type = '0', auditStatus = '0'): void {
    this.$router.push({
      path: '/market/order_approval/turn_carmodel/detail',
      query: {
  ...
  async mounted() {
    ...
    this.tableLabelType = 'MARKET_ORDER_APPROVAL_TURN_CARMODEL_LIST_LABEL';
    // this.queryList();
    this.initSetTableLabel();
    this.initSearch();
  }
