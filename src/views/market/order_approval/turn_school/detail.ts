...
export default class MarketOrderApprovalTurnSchoolDetail extends mixins(ctjtPaginationMixins, clearCacheMixins) {
  ...
  @Action('order/addApprovesChangeSchool') private addApprovesChangeSchool!: (data: any) => any;
  ...
  handleWatchType(val: number): void {
    this.formRules.receipt[0].required = val === 5;
  }
  ...
  formRules = {
    ...
    newModel: [
      { required: true, message: '请选择新班别', trigger: ['change', 'blur'] }
    ],
    receipt: [
  ...
  async hanldIdNoSelect(value: any) {
    ...
    const body = await this.queryOrderPaysList({ type: 5, idNo, name: '转校费用' });
  ...
  queryReceiptNoSearch(value: string, cb: any) {
    cb(this.receiptOptes);
  }
    ...
    (this.$refs.formDataRef as VueComponentParent).validate((valid: boolean) => {
      if (valid) {
        ...
        if (id) {
          ...
          this.addApprovesChangeSchool(this.formData).then(() => {
            this.goback();
  ...
  goback() {
    this.clearCache();
    this.$router.push({ path: '/market/order_approval/turn_school' });
  }
  ...
  async btnSubmit(val: number) {
    ...
    if (val === 4) {
      this.type = 2;
      const { idNo } = this.formData;
      const body = await this.queryOrderPaysList({ type: 5, idNo });
      this.receiptOptes = body || [];
    }
    if (val === 5) {
  ...
  queryDetail() {
    ...
    this.queryClassesInfoList({ type: 1 }).then((res: any) => {
      this.classessOpt = res;
    });
    if (Number(id) > 0) {
      this.queryApprovesDetail({ id, orderId, type: 6 }).then((res: any) => {
