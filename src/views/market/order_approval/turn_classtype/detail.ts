...
export default class MarketOrderApprovalTurnClasstypeDetail extends mixins(ctjtPaginationMixins, clearCacheMixins) {
  ...
  handleWatchType(val: number): void {
    this.formRules.receipt[0].required = val === 1;
  }
  ...
  async hanldIdNoSelect(value: any) {
    ...
    const body = await this.queryOrderPaysList({ type: 2, idNo });
    ...
    (this.$refs.formDataRef as VueComponentParent).validate((valid: boolean) => {
      if (valid) {
        ...
        if (id) {
          ...
          this.addApprovesChangeClasses(this.formData).then(() => {
            this.goback();
  ...
  goback() {
    this.clearCache();
    this.$router.push({ path: '/market/order_approval/turn_classtype' });
  }
  ...
  queryDetail() {
    ...
    if (Number(id) > 0) {
      this.queryApprovesDetail({ id, orderId, type: 2 }).then((res: any) => {
