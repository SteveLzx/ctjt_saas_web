...
export default class EducationalCoachMgCoachApplyDetail extends mixins(clearCacheMixins) {
  @Action('assignment/queryVerifyCoachChangeDetail') private queryVerifyCoachChangeDetail!: (data: any) => ParamsType;

  @Action('assignment/addCoachVerifyOperation') private addCoachVerifyOperation!: (data: any) => ParamsType;
  ...
  private async submit(code: number, verifyOpinion: string) {
    ...
    this.addCoachVerifyOperation(sendData).then(() => {
  ...
  private goback() {
    this.clearCache();
    this.$router.push({ path: '/educational/coach_mg/coach_apply' });
  }
