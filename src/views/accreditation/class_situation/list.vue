...
export default class ClassSituationList extends mixins(
  accreditationSeachTableMixins,
  ctjtPaginationMixins,
  ctjttablefieldMixins,
  accreditationOperationLogMixins
) {
  @Action('license/queryAttendClassList') private queryAttendClassList!: (
    data: any
  ) => any;

  @Action('license/attendClassExport') private attendClassExport!: (
    data: any
  ) => any;
  ...
  private tableOptionCallback(val: TableOptionsValue) {
    ...
    if (id === 13) {
      ...
      if (_len === 1) {
        ...
        const {
          userName, idNo, recordId, attendClassDate, remark
        } = selectionList[0];
        this.editData = {
          userName,
          idNo,
          id: recordId,
          applyDate: attendClassDate,
          remark,
          code: 10,
        };
        return;
  ...
  private async _exportData() {
    ...
    const body = await this.attendClassExport(sendData);
  ...
  async queryList() {
    ...
    try {
      const body = await this.queryAttendClassList(sendData);
  ...
  async mounted() {
    ...
    this.tableLabelType = 'CLASS_SITUATION_LIST_LABEL';
    this.queryList();
    this.initSearch();
    this.initSetTableLabel();
