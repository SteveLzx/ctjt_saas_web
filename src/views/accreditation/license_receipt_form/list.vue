...
export default class LicenseReceiptFormList extends mixins(
  accreditationSeachTableMixins,
  ctjtPaginationMixins,
  ctjttablefieldMixins,
  accreditationOperationLogMixins
) {
  @Action('license/queryRecoveryTableList') private queryRecoveryTableList!: (
    data: any
  ) => any;

  @Action('license/recoveryTableExport') private recoveryTableExport!: (
    data: any
  ) => any;
  ...
  private tableOptionCallback(val: TableOptionsValue) {
    ...
    if (id === 13) {
      ...
      if (_len === 1) {
        ...
        const {
          userName, idNo, recordId, recoveryDate
        } = selectionList[0];
        this.editData = {
          userName,
          idNo,
          id: recordId,
          applyDate: recoveryDate,
          code: 8,
        };
        return;
      }
  ...
  private async _exportData() {
    ...
    const body = await this.recoveryTableExport(sendData);
  ...
  async queryList() {
    ...
    try {
      const body = await this.queryRecoveryTableList(sendData);
  ...
  async mounted() {
    ...
    this.tableLabelType = 'LICENSE_RECEIPT_FORM_LIST_LABEL';
    this.queryList();
    this.initSearch();
    this.initSetTableLabel();
