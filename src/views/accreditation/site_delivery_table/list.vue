...
export default class SiteDeliveryTableList extends mixins(
  accreditationSeachTableMixins,
  ctjtPaginationMixins,
  ctjttablefieldMixins,
  accreditationOperationLogMixins
) {
  @Action('license/queryFieldPointList') private queryFieldPointList!: (
    data: any
  ) => any;

  @Action('license/fieldPointExport') private fieldPointExport!: (
    data: any
  ) => any;
  ...
  private initSetTableLabel() {
    const { tableLabelType, deletedLabels } = this; // 节点列表key名,删除数据特有字段
  ...
  private tableOptionCallback(val: TableOptionsValue) {
    ...
    if (id === 13) {
      ...
      if (_len === 1) {
        ...
        const {
          userName, idNo, recordId, deliverDate
        } = selectionList[0];
        this.editData = {
          userName,
          idNo,
          id: recordId,
          applyDate: deliverDate,
          code: 7,
        };
        return;
      }
  ...
  private async _exportData() {
    ...
    const body = await this.fieldPointExport(sendData);
  ...
  async queryList() {
    ...
    try {
      const body = await this.queryFieldPointList(sendData);
  ...
  async mounted() {
    ...
    this.tableLabelType = 'SITE_DELIVERY_TABLE_LIST_LABEL';
    this.queryList();
    this.initSearch();
    this.initSetTableLabel();
