<template>
  ...
  >
    <!--form 表单-->
    <CtjtCard :prop-data="{ title: '补录办证数据', tips: title }">
...
export default class CtjtSupplementDrawer extends Vue {
  ...
  @Prop({ default: null }) batchNo?: string;

  private title = '';

  @Watch('isShow')
  watchShowFun(val: boolean) {
    if (val) {
      this.$nextTick(() => {
        const { inputList } = this.commonForm;
        if (this.batchNo) {
          inputList[1].disabled = true;
          inputList[1].value = this.batchNo;
        } else {
  ...
  formCallBack(val: any) {
    ...
    if (key === 'add' && result) {
      ...
      const { drivingSchoolId } = supplementProps;
      const _data = drawSearchForm(commonForm);
      if (supplementProps.code === 'exam_acceptance') {
        if (
      ...
      if (supplementProps.code === 'exam_results') {
        if (
      ...
      if (
        supplementProps.code === 'exam_approval'
        || supplementProps.code === 'exam_fee'
        || supplementProps.code === 'exam_results'
      ) {
  ...
  /** 重置表单 */
  resetForm(disabled?: boolean, batchNo?: string) {
    (this.$refs.insert_form as VueComponentParent).resetFields();
    this._setBatchNoDisabled(disabled, batchNo);
    this._setExamAcceptDisabled(false);
    // 考试成绩可输入
    this._setExamResultOrExamGradeDisabled(false, 'examGrade');
    // 考试结果可输入
    this._setExamResultOrExamGradeDisabled(false, 'examResult');
    this._setCancelReasonIsRequired(false);
  }

  /** 设置补录时，批次号添加一次后不需要再手动添加 */
  _setBatchNoDisabled(disabled?: boolean, value?: string) {
    if (!this.batchNo) {
      const { inputList } = this.commonForm;
      inputList[1].disabled = disabled;
  ...
  private tableOptionCallback(val: TableOptionsValue) {
    ...
    if (id === 1) {
      ...
      if (_len >= 1) {
        ...
        if (this.previewData.list.length <= 0) {
          this._setBatchNoDisabled(false, '');
        }
      } else {
  ...
  async handleButtonClick(key?: string) {
    if (key === 'submit') {
      ...
      if (list.length > 0) {
        ...
        const { supplementPath } = this.supplementProps;
  ...
  selectChange(item: any) {
    ...
    if (_item.key === 'idNo') {
      _item.options = []; // 清空历史记录
      // 设置根据证件号拿到的用户信息
      (this.$refs.insert_form as VueComponentParent).clearValidate();
      this.teachingType = _list.length > 0 ? _list[0].teachingType : '';
    ...
    if (_item.key === 'examResult') {
      ...
      // 考试结果=取消考试时,取消原因必选
      if (examResult === '取消考试') this._setCancelReasonIsRequired(true);
    ...
    if (_item.key === 'examResultType') {
      // 设置考试不合格原因options
      this._setFailResonList(_item.value);
    }
  ...
  setFeeSubjectDisabled(subject: number) {
    ...
    selectList[3].options = feeSubjectList;
  }
