...
export default class PaperlessCollectList extends mixins(
  accreditationSeachTableMixins,
  ctjtPaginationMixins,
  ctjttablefieldMixins,
  accreditationOperationLogMixins
) {
  @Action('license/paperList') private paperList!: (data: any) => any;

 @Action('license/paperExport') private paperExport!: (data: any) => any;
  ...
  private tableOptionCallback(val: TableOptionsValue) {
    ...
    if (id === 13) {
      ...
      if (_len === 1) {
        ...
        const {
          userName, idNo, recordId, collectDate
        } = selectionList[0];
        this.editData = {
          userName,
          idNo,
          id: recordId,
          applyDate: collectDate,
          code: 1,
        };
        return;
      }
  ...
  private async _exportData() {
    ...
    const body = await this.paperExport(sendData);
  ...
  async queryList() {
    ...
    try {
      const body = await this.paperList(sendData);
  ...
  async mounted() {
    ...
    this.tableLabelType = 'PAPERLESS_COLLECT_LIST_LABEL';
    this.queryList();
    this.initSearch();
    this.initSetTableLabel();
