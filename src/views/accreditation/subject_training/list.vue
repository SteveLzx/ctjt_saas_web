...
export default class AccreditationSubjectTrainingList extends mixins(
  accreditationSeachTableMixins,
  ctjtPaginationMixins,
  ctjttablefieldMixins,
  accreditationOperationLogMixins
) {
  @Action('license/querySubjectTrainsList') private querySubjectTrainsList!: (
    data: any
  ) => any;

  @Action('license/subjectTrainsExport') private subjectTrainsExport!: (
    data: any
  ) => any;
  ...
  private async _exportData() {
    ...
    const body = await this.subjectTrainsExport(sendData);
  ...
  async queryList() {
    ...
    try {
      const body = await this.querySubjectTrainsList(sendData);
  ...
  async mounted() {
    ...
    this.tableLabelType = 'SUBJECT_TRAINING_LIST_LABEL';
    this.queryList();
    this.initSearch();
    this.initSetTableLabel();
