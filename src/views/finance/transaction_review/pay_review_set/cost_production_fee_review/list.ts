...
const name = '工本费复核';
@Component
export default class FinancepayReviewSetCostProductionFeeReview extends mixins(ctjtPaginationMixins, ctjttablefieldMixins, financePayReviewSeachTableMixins) {
  ...
  // 列表搜索项配置
  private initLocalSearchForm() {
    const { selectList } = this.searchForm;
    selectList[2].display = 'none';
  }
 ...
 */
  ...
  */
  ...
  // private _queryStaticData() {
  //   const { list } = this.tableData;
  //   const _list = deepClone(list);
  //   const notReviewStatus = REVIEW_STATUS[0].id; // 待复核id
  //   const notReviewList = _list.filter((a: any) => a.reviewStatus === notReviewStatus);// 待复核list
  //   if (_list && _list.length > 0 && notReviewList.length > 0) {
  //     const notReviewCount = notReviewList ? notReviewList.length : 0;
  //     // 金额总和
  //     const notReviewMoney = notReviewCount > 1 ? notReviewList.reduce((sumVaule: any, currentValue: any) => {
  //       const sum = isCustomNumber(sumVaule) ? sumVaule : sumVaule.fee;
  //       return currentValue.fee ? (sum + currentValue.fee) : sum;
  //     }) : notReviewList[0].fee;
  //     this.statisticsData[0].value = notReviewCount;
  //     this.statisticsData[1].value = notReviewMoney;
  //   } else {
  //     this.statisticsData[0].value = 0;
  //     this.statisticsData[1].value = 0;
  //   }
  // }
  ...
  async checkTab(val: number) {
    ...
    const _typeDta: ParamsType = {
      1: 'COST_PRODUCTION_FEE_REVIEW_OTHER_LIST_LABEL',
      2: 'COST_PRODUCTION_FEE_REVIEW_SELF_LIST_LABEL',
    };
  ...
  */
  ...
  async mounted() {
    ...
    this.queryList();
    this.tableLabelType = 'COST_PRODUCTION_FEE_REVIEW_OTHER_LIST_LABEL';
    this.initSetTableLabel();
    this.initLocalSearchForm();
  }
