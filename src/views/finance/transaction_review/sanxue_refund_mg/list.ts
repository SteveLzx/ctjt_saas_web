...
export default class FinanceStudentRefundMg extends mixins(ctjtPaginationMixins, ctjttablefieldMixins) {
  @Action('finance/querySanXueRefundCountList') private querySanXueRefundCountList!: (data: any) => ParamsType;
  ...
  */
  ...
  private async _exportData(selectionList: any) {
    ...
    FileSaver.saveAs(blob, `散学退费明细${this.$dayjs(new Date()).format('YYYYMMDD')}`);
  }
 ...
 */
  jumpDetail(val: ParamsType) {
    this.$router.push({
      path: '/finance/transaction_review/sanxue_refund_mg/detail',
 ...
 */
  ...
  async queryList() {
    ...
    try {
      const body = await this.querySanXueRefundCountList(sendData);
  ...
  mounted() {
    ...
    this.queryList();
    this.tableLabelType = 'SANXUE_REFUND_MG_LIST_LABEL';
    this.initSetTableLabel();
  }
