...
const name = '散学收款明细';
@Component
export default class FinanceCollectionReviewSetSanXue extends mixins(ctjtPaginationMixins, clearCacheMixins, ctjtAreaStoreSeachTableMixins) {
  @Action('finance/querySanXueDetailList') private querySanXueDetailList!: (data: any) => ParamsType;

  @Action('finance/querySanXueDetailCount') private querySanXueDetailCount!: (data: any) => ParamsType;
  ...
  */
  ...
  private async _exportData() {
    ...
    const fileName = type === 1 ? '散学收款金额明细汇总' : name;
    const sendData = {
      payType,
      account,
      beginDate,
    ...
    const body = await await this.querySanXueDetailExport(sendData);
  ...
  async queryList() {
    ...
    try {
      const body = await this.querySanXueDetailList(sendData);
  ...
  async getStaticData(sendData: any) {
    try {
      const body = await this.querySanXueDetailCount(sendData);
  ...
  async created() {
    ...
    if (typeof obj === 'string') {
      ...
      const { drivingSchoolId } = this.userInfo;
      await this.queryRegionList(drivingSchoolId); // 广仁要设置默认片区
      this.initSearch();
      this.queryList();
