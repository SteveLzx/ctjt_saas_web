...
export default class FinanceConfirmIncomeByStageOtherRefund extends mixins(ctjtPaginationMixins) {
  @Action('finance/queryOtherIncomeListCount') private queryOtherIncomeListCount!: (data: any) => any;

  @Action('finance/exportOtherIncomeList') private exportOtherIncomeList!: (data: any) => any;

  @Action('finance/confirmedOtherIncome') private confirmedOtherIncome!: (data: any) => any;

  @Action('finance/queryOtherIncomeListDetail') private queryOtherIncomeListDetail!: (data: any) => any;
  ...
  searchTableCallBack(key: string) {
    ...
    confirm() {
      ...
      this.confirmedOtherIncome(sendData).then(() => {
    ...
    async exportData(type: number, idList?: Array<string>) {
      ...
      const body = await this.exportOtherIncomeList(sendData);
      const blob = new Blob([body], { type: 'application/vnd.ms-excel' });
      const _fileName = type === 1 ? '其他收入确认汇总' : '其他收入确认明细';
    ...
    queryList() {
      ...
      this.queryOtherIncomeListCount(sendData).then((res: any) => {
    ...
    queryListDrawer() {
      ...
      sendData = { ...sendData, ...drawerSearchForm };
      this.queryOtherIncomeListDetail(sendData).then((res: any) => {
