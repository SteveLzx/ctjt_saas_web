...
import { API_FINANCE_V1_STAGEINCOMECOUNT_HUIZHOU_REFUND_EXPORT } from '@/api';
import ctjtPaginationMixins from '@/mixins/pagination';

const name = '本期退费';
@Component
export default class FinanceHuiZhouCurrentRefund extends mixins(ctjtPaginationMixins) {
  @Action('finance/queryHuiZhouCurrentRefundList') private queryHuiZhouCurrentRefundList!: (data: any) => ParamsType;
    ...
    labels: [
      ...
      },
      {
        ...
        render(h: any, params: any) {
          ...
          return h('div', dayjs(refundDate).format('YYYY-MM-DD'));
        }
      },
      {
        key: 'trainCost',
  ...
  */
  ...
  private async _exportData() {
    ...
    const body = await this.$http.post(API_FINANCE_V1_STAGEINCOMECOUNT_HUIZHOU_REFUND_EXPORT, sendData, {
  ...
  async queryList() {
    ...
    try {
      const body = await this.queryHuiZhouCurrentRefundList(sendData);
