...
import { jsAddFunc, isCustomNumber, formatPrice } from '@/assets/js/common';
import { SUBJECT } from '@/enums';

@Component
export default class FinanceGraduationCorrection extends Vue {
  @Action('finance/queryGraduationCorrectionList') private queryGraduationCorrectionList!: (data: any) => ParamsType;

  @Action('finance/confirmIncomGraduationCorrection') private confirmIncomGraduationCorrection!: (data: any) => ParamsType;
  ...
  */
  ...
  private confirm() {
    ...
    this.confirmIncomGraduationCorrection(sendData).then(() => {
      this.queryList();
  ...
  jumpDetail(step: number) {
    ...
    this.$router.push({
      path: '/finance/confirm_income_by_stage/graduation_correction/detail',
  ...
  async queryList() {
    ...
    const sendData = drawSearchForm(searchForm);
    try {
      const body = await this.queryGraduationCorrectionList(sendData);
