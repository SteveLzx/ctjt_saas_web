...
import { API_FINANCE_V1_GRADUATE_EXPORTGRADUATECURINGDETAIL } from '@/api';
...
@Component
export default class FinanceCurrentGraduationFixedDetail extends mixins(ctjtPaginationMixins, clearCacheMixins) {
  @Action('finance/queryCurrentGraduateFixedDetail') private queryCurrentGraduateFixedDetail!: (data: any) => ParamsType;
  ...
  */
  ...
  private async _exportData() {
    ...
    const body = await this.$http.post(API_FINANCE_V1_GRADUATE_EXPORTGRADUATECURINGDETAIL, sendData, {
  ...
  async queryList() {
    ...
    try {
      const body = await this.queryCurrentGraduateFixedDetail(sendData);
