...
import { API_FINANCE_V1_GRADUATIONCORRECTION_EXPORTGRADUATECORRECTIONDETAIL } from '@/api';
import ctjtPaginationMixins from '@/mixins/pagination';
import clearCacheMixins from '@/mixins/clearCache';

const name = '修正明细（固化）';
@Component
export default class FinanceGraduationCorrectionFixedDetail extends mixins(ctjtPaginationMixins, clearCacheMixins) {
  @Action('finance/queryGraduationCorrectionFixedDetail') private queryGraduationCorrectionFixedDetail!: (data: any) => ParamsType;
  ...
  */
  ...
  private async _exportData() {
    ...
    const body = await this.$http.post(API_FINANCE_V1_GRADUATIONCORRECTION_EXPORTGRADUATECORRECTIONDETAIL, sendData, {
  ...
  async queryList() {
    ...
    try {
      const body = await this.queryGraduationCorrectionFixedDetail(sendData);
